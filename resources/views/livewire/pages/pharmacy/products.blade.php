<div>
    <x-slot name="header">
        <div class="flex justify-between" x-data="{ open: false, toggle() { this.open = !this.open } }">
            ...
            <x-dropdown>
                <x-slot name="trigger">
                    <button class="headerStyle-link text-white">Edit a Price</button>
                </x-slot>
                <x-slot name="content" class="contentStyle">
                    <livewire:components.create-product class="items-center"/>
                </x-slot>
            </x-dropdown>
            {{--            <div class="relative">--}}
            {{--                <div x-cloak--}}
                        ...
                        @foreach($products->all() as $product)
                            <div wire:key="{{ $product->id }}">
                                <livewire:components.ModelListItem
                                    ...
                                    :model_id="$product->id"
                                    modelName="contract"
                                    :topLeft="$product->tradeName"
                                    :bottomLeft="$product->formula"
                                    :topRight="'$'.$product->price"
                                    :bottomRight="$product->created_at"
                                    :useDelete="true"
                                />
                            </div>
                        @endforeach
